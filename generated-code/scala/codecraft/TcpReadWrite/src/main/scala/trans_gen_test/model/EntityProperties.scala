 ...
 */
case class EntityProperties(size: Int, buildScore: Int, destroyScore: Int, canMove: Boolean, populationProvide: Int, populationUse: Int, maxHealth: Int, initialCost: Int, sightRange: Int, resourcePerHealth: Int, build: Option[trans_gen_test.model.BuildProperties], attack: Option[trans_gen_test.model.AttackProperties], repair: Option[trans_gen_test.model.RepairProperties]) {
    /**
...
object EntityProperties {
    ...
    def readFrom(stream: java.io.InputStream): EntityProperties = EntityProperties(
        ...
        if (StreamUtil.readBoolean(stream)) Some(
            trans_gen_test.model.BuildProperties.readFrom(stream)
        ) else None,
        if (StreamUtil.readBoolean(stream)) Some(
            trans_gen_test.model.AttackProperties.readFrom(stream)
        ) else None,
        if (StreamUtil.readBoolean(stream)) Some(
            trans_gen_test.model.RepairProperties.readFrom(stream)
        ) else None
    )
}